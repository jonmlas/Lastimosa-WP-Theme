...
if ( ! function_exists( 'delete_shortcode_icon_box_style_temp' ) ) :
	/**
	 * Resets the icon-box-style-temp to null on page load
	 * which means it will remove the css style of any deleted shortcode
	 */
	function delete_shortcode_icon_box_style_temp() {
		delete_option( 'icon-box-style-temp' );	
	}
	if( get_option( 'icon-box-style-temp' ) ) {
		delete_shortcode_icon_box_style_temp();
	}
endif;
